...
sys_mbox_trypost (sys_mbox_t *mbox, void *msg)
{
    ...
    /*
     * Things work much better when the  sem_trywait(&mbox->mutex) is sem_wait(&mbox->mutex) instead.
     * This function is not being called from an interrupt handler so a wait on the mutex should be just fine.

     * With the sem_trywait() the function returns with an error if the mutex is already taken. 
     * When this happens, the msg being put into the mbox is dropped. 
     * This can happen quite often and results in many TCP retransmissions and generally bad performance. 
     * The intent was for the function to return with an error if the mbox is full, 
     * not because the mutex needed to check the mbox is temporarily busy.
     */
