...
proc generate_lwip_opts {libhandle} {
    ...
    puts $lwipopts_fd ""
    puts $lwipopts_fd "\#include \<limits.h\>"
    puts $lwipopts_fd ""
    puts $lwipopts_fd "/* Functions used to obtain and release exclusive access to the Tx buffer. The"
    puts $lwipopts_fd "Get function will block if the Tx buffer is not available - use with care! */"
    puts $lwipopts_fd "signed char *pcLwipBlockingGetTxBuffer( void );"
    puts $lwipopts_fd "void vLwipAppsReleaseTxBuffer( void );"
    puts $lwipopts_fd ""
    ...
    if {$api_mode == "SOCKET_API"} {
        ...
        puts $lwipopts_fd "\#define LWIP_COMPAT_SOCKETS 1"
        puts $lwipopts_fd "\#define LWIP_NETCONN 1"
    }
    puts $lwipopts_fd ""
    ...
    if {$api_mode == "SOCKET_API"} {
        ...
        if { [string compare -nocase "freertos_zynq" $os_name] == 0} {
            ...
            puts $lwipopts_fd "\#define TCPIP_MBOX_SIZE 10"
    ...
    # IGMP options
    set lwip_igmp   [expr [get_property CONFIG.lwip_igmp $libhandle] == true]
    puts $lwipopts_fd "\#define LWIP_SNMP 0"
    puts $lwipopts_fd "\#define LWIP_IGMP $lwip_igmp"
    puts $lwipopts_fd ""

    # lwIP debug
    set lwip_debug      [expr [get_property CONFIG.lwip_debug $libhandle] == true]
    set ppp_debug       [expr [get_property CONFIG.ppp_debug $libhandle] == true]
    set mem_debug       [expr [get_property CONFIG.mem_debug $libhandle] == true]
    set memp_debug      [expr [get_property CONFIG.memp_debug $libhandle] == true]
    ...
    set tcpip_debug     [expr [get_property CONFIG.tcpip_debug $libhandle] == true]
    set netif_debug     [expr [get_property CONFIG.netif_debug $libhandle] == true]
    set sockets_debug   [expr [get_property CONFIG.sockets_debug $libhandle] == true]
    set dns_debug       [expr [get_property CONFIG.dns_debug $libhandle] == true]
    set autoip_debug    [expr [get_property CONFIG.autoip_debug $libhandle] == true]
    set dhcp_debug      [expr [get_property CONFIG.dhcp_debug $libhandle] == true]
    ...
    if {$lwip_debug == 1} {
        ...
        puts $lwipopts_fd "\#define LWIP_DBG_MIN_LEVEL LWIP_DBG_LEVEL_ALL"
        puts $lwipopts_fd "\#define DBG_TYPES_ON DBG_LEVEL_WARNING"
        if {$ppp_debug} {
            puts $lwipopts_fd "\#define PPP_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)"
        }
        if {$mem_debug} {puts $lwipopts_fd "\#define MEM_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)"}
        if {$memp_debug} {puts $lwipopts_fd "\#define MEMP_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)"}
        if {$pbuf_debug} {
        ...
        if {$sys_debug} {
            ...
            puts $lwipopts_fd "\#define API_LIB_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)"
            puts $lwipopts_fd "\#define API_MSG_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)"
        }
        if {$tcpip_debug} { puts $lwipopts_fd "\#define TCPIP_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" }
        if {$netif_debug} { puts $lwipopts_fd "\#define NETIF_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" }
        if {$sockets_debug} { puts $lwipopts_fd "\#define SOCKETS_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" }
        if {$dns_debug} { puts $lwipopts_fd "\#define DNS_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" }
        if {$autoip_debug} { puts $lwipopts_fd "\#define AUTOIP_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" }
        if {$dhcp_debug} { puts $lwipopts_fd "\#define DHCP_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" }
        if {$ip_debug} {
            puts $lwipopts_fd "\#define IP_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)"
            puts $lwipopts_fd "\#define IP_REASS_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)"
        }
        ...
        if {$tcp_debug} { 
            ...
            puts $lwipopts_fd "\#define TCP_RTO_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" 
            puts $lwipopts_fd "\#define TCP_CWND_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" 
            puts $lwipopts_fd "\#define TCP_WND_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" 
            puts $lwipopts_fd "\#define TCP_FR_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" 
            puts $lwipopts_fd "\#define TCP_QLEN_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" 
            puts $lwipopts_fd "\#define TCP_RST_DEBUG (LWIP_DBG_LEVEL_SEVERE | LWIP_DBG_ON)" 
        }
        puts $lwipopts_fd ""
    }
    puts $lwipopts_fd "\#define LWIP_DBG_TYPES_ON (LWIP_DBG_ON | LWIP_DBG_TRACE | LWIP_DBG_STATE | LWIP_DBG_FRESH | LWIP_DBG_HALT)"
    puts $lwipopts_fd ""
    ...
    puts $lwipopts_fd "\#define LWIP_RAW $lwip_raw"
    puts $lwipopts_fd "\#define MEMP_NUM_RAW_PCB $memp_n_raw_pcb"
    ...
    puts $lwipopts_fd ""
    # always use lightweight prot mechanism for critical regions (NO_SYS==0)
    ...
    set arp_queueing    [expr [get_property CONFIG.arp_queueing $libhandle] == true]
    puts $lwipopts_fd "\#define LWIP_ARP $lwip_arp"
    puts $lwipopts_fd "\#define ARP_TABLE_SIZE $arp_table_size"
    puts $lwipopts_fd "\#define ARP_QUEUEING $arp_queueing"
    puts $lwipopts_fd ""

    # IP options
    set ip_forward          [expr [get_property CONFIG.ip_forward $libhandle] == true]
    set ip_options          [get_property CONFIG.ip_options $libhandle]
    set ip_reassembly       [expr [get_property CONFIG.ip_reassembly $libhandle] == true]
    set ip_reass_max_pbufs  [get_property CONFIG.ip_reass_max_pbufs $libhandle]
    set memp_num_reassdata  [get_property CONFIG.memp_num_reassdata $libhandle]
    set ip_frag             [expr [get_property CONFIG.ip_frag $libhandle] == true]
    ...
    puts $lwipopts_fd "\#define MEMP_NUM_REASSDATA $memp_num_reassdata"
    ...
    # ICMP options
    set lwip_icmp [expr [get_property CONFIG.lwip_icmp $libhandle] == true]
    set icmp_ttl [get_property CONFIG.icmp_ttl $libhandle]
    puts $lwipopts_fd "\#define LWIP_ICMP $lwip_icmp"
    ...
    set lwip_autoip [expr [get_property CONFIG.lwip_autoip $libhandle] == true]
    set lwip_dhcp_autoip_coop [expr [get_property CONFIG.lwip_dhcp_autoip_coop $libhandle] == true]
    puts $lwipopts_fd "\#define LWIP_DHCP $lwip_dhcp"
    puts $lwipopts_fd "\#define DHCP_DOES_ARP_CHECK $dhcp_does_arp_check"
    puts $lwipopts_fd "\#define LWIP_AUTOIP $lwip_autoip"
    puts $lwipopts_fd "\#define LWIP_DHCP_AUTOIP_COOP $lwip_dhcp_autoip_coop"
    puts $lwipopts_fd ""

    # UDP options
    set lwip_udp [expr [get_property CONFIG.lwip_udp $libhandle] == true]
    set lwip_udplite [expr [get_property CONFIG.lwip_udplite $libhandle] == true]
    set udp_ttl [get_property CONFIG.udp_ttl $libhandle]
    puts $lwipopts_fd "\#define LWIP_UDP $lwip_udp"
    puts $lwipopts_fd "\#define LWIP_UDPLITE $lwip_udplite"
    ...
    set icmp_stats   [expr [get_property CONFIG.icmp_stats $libhandle] == true]
    set igmp_stats   [expr [get_property CONFIG.igmp_stats $libhandle] == true]
    set ipfrag_stats [expr [get_property CONFIG.ipfrag_stats $libhandle] == true]
    set udp_stats    [expr [get_property CONFIG.udp_stats $libhandle] == true]
    set tcp_stats    [expr [get_property CONFIG.tcp_stats $libhandle] == true]
    set mem_stats    [expr [get_property CONFIG.mem_stats $libhandle] == true]
    set memp_stats   [expr [get_property CONFIG.memp_stats $libhandle] == true]
    set pbuf_stats   [expr [get_property CONFIG.pbuf_stats $libhandle] == true]
    set sys_stats    [expr [get_property CONFIG.sys_stats $libhandle] == true]
    puts $lwipopts_fd "\#define LWIP_STATS $lwip_stats"
    puts $lwipopts_fd "\#define LWIP_STATS_DISPLAY $lwip_stats_display"
    if {$lwip_stats} {
        puts $lwipopts_fd "\#define LINK_STATS $link_stats"
        puts $lwipopts_fd "\#define IP_STATS $ip_stats"
        puts $lwipopts_fd "\#define ICMP_STATS $icmp_stats"
        puts $lwipopts_fd "\#define IGMP_STATS $igmp_stats"
        puts $lwipopts_fd "\#define IPFRAG_STATS $ipfrag_stats"
        puts $lwipopts_fd "\#define UDP_STATS $udp_stats"
        puts $lwipopts_fd "\#define TCP_STATS $tcp_stats"
        puts $lwipopts_fd "\#define MEM_STATS $mem_stats"
        puts $lwipopts_fd "\#define MEMP_STATS $memp_stats"
        puts $lwipopts_fd "\#define PBUF_STATS $pbuf_stats"
        puts $lwipopts_fd "\#define SYS_STATS $sys_stats"
        puts $lwipopts_fd ""
    }

    # PPP options
    set lwip_ppp       [expr [get_property CONFIG.lwip_ppp $libhandle] == true]
    set num_ppp        [expr [get_property CONFIG.num_ppp $libhandle] == true]]
    ...
    # enable neif callback
    puts $lwipopts_fd "\#define LWIP_NETIF_STATUS_CALLBACK 1"
    puts $lwipopts_fd ""
